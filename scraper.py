...
# Disable SSL warning messages in GitHub Actions console
...
OPEN_INDICATORS = ["inscrições abertas", "inscreva-se", "edital aberto", "prorrogado", "vagas abertas"]
CLOSED_INDICATORS = ["encerradas", "inscrições encerradas", "edital encerrado", "finalizado"]
...
def fetch_page_html(url):
    """Safely fetches page HTML, bypassing SSL verification issues on older servers."""
    try:
...
def extract_academic_links(html, base_url):
    """Parses HTML, finds keyword-matching links, skips binary files."""
...
def analyze_program_status(html_content):
    """Determines admission status and deadline from page HTML."""
    if not html_content:
    ...
    # Status detection
    if any(ind in html_lower for ind in OPEN_INDICATORS):
        status = "Open"
    elif any(ind in html_lower for ind in CLOSED_INDICATORS):
        status = "Closed"
    else:
        status = "Check Portal"

    # Deadline extraction — look for DD/MM/YYYY patterns
    deadline = "See Website"
    import re
    date_matches = re.findall(r'\b(\d{2}/\d{2}/(?:\d{4}|\d{2}))\b', html_content)
    if date_matches:
        deadline = date_matches[-1]

    return status, deadline


def run_scraper_engine():
    """Main pipeline: crawl all universities, extract programs, save JSON."""
    # Load existing data to preserve entries for offline sites (fail-soft)
    existing_data = {}
    if os.path.exists("discovered_programs.json"):
        try:
            with open("discovered_programs.json", "r", encoding="utf-8") as f:
                old = json.load(f)
                for item in old:
                    existing_data[item.get("link", "")] = item
        except Exception:
            pass

    print(f"🚀 Crawling {len(university_directory)} university targets...\n")
    compiled_results = []
    uid = 1

    for uni in university_directory:
        print(f"  🔍 {uni['name']} ({uni['state']})")
        page_html = fetch_page_html(uni['url'])

        if not page_html:
            # Preserve old entry if site is temporarily down
            old_entry = existing_data.get(uni['url'])
            if old_entry:
                compiled_results.append(old_entry)
            continue

        found_links = extract_academic_links(page_html, uni['url'])

        if found_links:
            primary = found_links[0]
            deep_html = fetch_page_html(primary['link'])
            status, deadline = analyze_program_status(deep_html or page_html)

            entry = {
                "id": uid,
                "university": uni['name'],
                "program": primary['title'] if len(primary['title']) > 5 else "Processo Seletivo Regular",
                "region": uni['region'],
                "state": uni['state'],
                "status": status,
                "deadline": deadline,
                "link": primary['link'],
                "type": "Federal Public" if any(x in uni['name'] for x in ["UF", "UFPA", "UFMG", "UFRJ", "UnB", "UFC"]) else "State Public"
            }
            compiled_results.append(entry)
            print(f"     ✅ Status: {status} | Deadline: {deadline}")
            uid += 1
        else:
            # No deep links found — use the portal itself as the entry
            entry = {
                "id": uid,
                "university": uni['name'],
                "program": f"Postgraduate Portal – {uni['name']}",
                "region": uni['region'],
                "state": uni['state'],
                "status": "Check Portal",
                "deadline": "See Website",
    ...
    with open("discovered_programs.json", "w", encoding="utf-8") as f:
        json.dump(compiled_results, f, ensure_ascii=False, indent=2)

    print(f"\n🎉 Done! {len(compiled_results)} programs saved to discovered_programs.json")
